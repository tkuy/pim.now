...
import { defaultValue, IWorkflow } from 'app/shared/model/workflow.model';

export const ACTION_TYPES = {
  SEARCH_WORKFLOWS: 'workflow/SEARCH_WORKFLOWS',
...
export default (state: WorkflowState = initialState, action): WorkflowState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_WORKFLOWS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_WORKFLOWS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_WORKFLOWS):
...
const apiSearchUrl = 'api/_search/workflows';

// Actions

export const getSearchEntities: ICrudSearchAction<IWorkflow> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_WORKFLOWS,
  payload: axios.get<IWorkflow>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`)
});
